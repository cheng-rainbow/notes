...
2. **`表级锁`**：
   - **共享锁（Shared Lock，S Lock）**：允许多个事务同时`读取`同一资源，但禁止任何事务对该数据进行修改。  （select）（`LOCK IN SHARE MODE`）
   - **排他锁（Exclusive Lock，X Lock）**：锁定资源后，只有持有锁的事务`可以读写`，其他事务`无法读写`。  （update、delete）（`FOR UPDATE`）
   - **元数据锁（Metadata Lock，MDL）**：它确保在事务执行期间，数据库对象的`结构信息`（列定义、索引、约束等）不会被其他并发操作修改，从而维护事务的一致性和稳定性。（事务A正在查询表users，事务B试图执行`ALTER TABLE users ADD COLUMN new_col`。**MDL确保**事务A完成前，事务B的DDL**被阻塞**。）
   - **意向锁（Intention Lock）**：在锁定某行之前，先在表上加相应的意向锁。若已有表级X锁，意向锁请求被阻塞。若已有表级S锁，IX锁请求被阻塞，IS锁可通过。事务提交或回滚时，意向锁和行锁一起释放。
3. 按锁的算法分类（InnoDB 特有）
