...
1. `程序计数器`：程序计数器记录当前线程正在执行的 `JVM 指令` 的偏移量，帮助 JVM 定位字节码指令并执行相应操作。
2. `虚拟机栈`：虚拟机栈（JVM Stack） 是 Java 虚拟机为每个 **线程** 分配的私有内存空间，生命周期与线程相同。它主要用于管理 Java **方法** 的执行。
3. `堆`：堆（Heap）用于存储所有 **对象实例** 和 **数组**，被所有线程共享。
